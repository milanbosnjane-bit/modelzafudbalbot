"""Model B: LightGBM regressor — predicts expected return per unit staked."""
...
class LightGBMModel:
    ...
    def __init__(self, model_path: Path | None = None):
        self.model_path = model_path or settings.model_dir / "lightgbm.pkl"
    ...
    def train(
        ...
        self.model = lgb.LGBMRegressor(objective="regression", metric="rmse", verbose=-1, **params)
        self.model.fit(X, y)
        ...
        return {"model": "lightgbm", "samples": len(y), "params": params}
    ...
    def _optimize_hyperparams(self, X: np.ndarray, y: np.ndarray) -> dict:
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            for train_idx, val_idx in tscv.split(X):
                model = lgb.LGBMRegressor(objective="regression", verbose=-1, **params)
    ...
    def load(self) -> bool:
        path = resolve_trained_model("lightgbm.pkl")
        if path is None:
            return False
